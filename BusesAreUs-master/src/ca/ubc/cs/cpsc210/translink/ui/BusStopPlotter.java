...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        {
            Marker marker;
            if(this.stopMarkerMap.containsKey(stop))
                marker = this.getMarker(stop);
            else
            {
                marker = new Marker(mapView);
                marker.setIcon(stopIconDrawable);
                marker.setPosition(new GeoPoint(stop.getLocn().getLatitude(), stop.getLocn().getLongitude()));
                marker.setTitle(this.createStopInfo(stop));
                marker.setRelatedObject(stop);
                marker.setInfoWindow(this.stopInfoWindow);
                this.setMarker(stop, marker);
            }
            if(Geometry.rectangleContainsPoint(super.northWest, super.southEast, stop.getLocn()))
                this.stopClusterer.add(marker);
        }
        this.stopClusterer.invalidate();
    }

    private String createStopInfo(Stop stop)
    {
        String info = stop.getNumber() + " " + stop.getName();
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if(this.nearestStnMarker != null)
            this.nearestStnMarker.setIcon(stopIconDrawable);
        if(nearest == null)
            this.nearestStnMarker = null;
        else
        {
            this.nearestStnMarker = this.getMarker(nearest);
